    ...
    for transaction in transactions:
        if transaction.get("description"):
            ...
            try:
                ...
                if match:
                    logger.debug(
                        f"get_search_transactions: Запись id '{transaction['id']}' "
                        f"соответствует поисковой строке '{search_str}'"
                    )
                    search_result.append(transaction)
            except Exception as e:
                logger.error(
                    f"get_search_transactions: Исключение обработки '{e}' записи id '{transaction.get('id')}'"
                )
        ...
        if transaction.get("description"):
            ...
            try:
                if transaction["description"].lower() in lowercase_categories:
                    logger.debug(
                        f"get_transactions_count: Запись id : {transaction['id']} "
